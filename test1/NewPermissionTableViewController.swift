//
//  NewPermissionTableViewController.swift
//  test1
//
...
// Esta es la clase que maneja la pantalla de la creación de un permiso nuevo
class NewPermissionTableViewController: UITableViewController {
    // En esta variable podemos meter una función externa, para que cuando se cree un permiso se entregue por esta vía
    var callback: (Permission)->Void = { _ in }
    ...
    @IBOutlet weak var Permisos: UITextField!
    @IBOutlet weak var Proyecto: UITextField!
    ...
    @IBAction func cancel(_ sender: AnyObject) {
        // si se apreta cancelar, descartamos (dismiss) la pantalla de creación de permisos nuevos
    ...
    @IBAction func done(_ sender: AnyObject) {
        ...
        // Creamos un objeto Permission con los textos que 
        ...
        // hacemos un request de tipo POST a permissions.php
        Alamofire.request("\(apiUrl)/permissions.php", method: .post, parameters: parameters)
            ...
            .responseJSON { [unowned self] response in
                ...
                case .success(let data):
                    print(data)
                    SVProgressHUD.showSuccess(withStatus: "Permiso creado")
                    // en caso de éxito, llamamos a la función que nos pasaron, con el objeto permission
                    self.callback(permission)
